...
```
[x] Task 1: Tambah skema database Fase 2 (features, tasks, task_dependencies) + migrasi
[x] Task 2: Buat prompt template feature-breakdown.ts dan task-breakdown.ts
[x] Task 3: Implementasi API route POST /api/plans/[planId]/features/generate
[x] Task 4: Implementasi CRUD API route untuk Feature (edit/hapus/reorder)
[x] Task 5: Implementasi API route POST /api/features/[featureId]/tasks/generate (dengan Zod validation + dependency resolution)
[x] Task 6: Implementasi API route PATCH /api/tasks/[taskId]/status (dengan validasi dependency)
[x] Task 7: Implementasi API route GET /api/plans/[planId]/tasks/next (sebagai reusable function, bukan cuma route)
[x] Task 8: Buat komponen Feature List UI (dengan generate & edit)
[x] Task 9: Buat komponen Task Board (kanban drag-and-drop status)
[x] Task 10: Tambah tombol "Copy prompt untuk agent" di setiap task card
[x] Task 11: Testing end-to-end: PRD final → generate features → generate tasks → update status → ambil next task
```

## 11. Kriteria Selesai (Definition of Done — Fase 2)

- [x] Dari satu PRD, sistem bisa generate daftar Features yang selaras dengan `coreFeatures`
- [x] Setiap Feature bisa dipecah jadi Tasks dengan urutan frontend→backend yang masuk akal
- [x] Task dengan dependency yang belum selesai tidak bisa diubah ke status `doing`
- [x] Endpoint `tasks/next` mengembalikan task yang benar-benar siap dikerjakan
- [x] Fungsi `tasks/next` dibuat reusable (dipanggil langsung sebagai function, bukan hanya lewat HTTP) agar siap dipakai MCP Server di Fase 3
